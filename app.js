...
const sampleManga = [
  {
    ...
    author: "吉田秋生",
    genre: "治愈 / 日常",
    quote: "生活不总是轰轰烈烈，温柔本身就足够有力量。",
    description:
      "四姐妹住在临海的旧屋中，在四季流转与家常饭菜里，慢慢理解家庭、离别和成长。\n\n安静的分镜和细腻的人物关系，让每一次阅读都像吹过一阵温柔的海风。",
  ...
  },
  {
    ...
    author: "山口飞翔",
    genre: "青春 / 艺术",
    quote: "喜欢的事情，也可以成为认真生活的理由。",
    description:
      "成绩优秀、处世圆滑的高中生矢口八虎，偶然在美术室看见一幅画，从此第一次对某件事产生真正的热情。\n\n它诚实地描写天赋、努力与自我怀疑，也记录一个人如何找到属于自己的表达方式。",
    coverTheme: "cover-night",
    local: false,
  },
  {
    id: "sample-3",
    ...
    author: "高松美咲",
    genre: "校园 / 喜剧",
    quote: "笨拙也没关系，真诚的人总能照亮彼此。",
    description:
      "从乡下来到东京读高中的岩仓美津未，带着一点莽撞和很多认真，认识了性格各异的新朋友。\n\n轻盈可爱的校园故事里，每个人的敏感与善意都被认真看见。",
    coverTheme: "cover-pink",
    local: false,
  },
  {
    id: "sample-4",
    ...
    author: "天野梢",
    genre: "科幻 / 治愈",
    quote: "平凡的日子里，藏着许多值得停下来看的奇迹。",
    description:
      "在被改造成水之星的火星上，少女水无灯里努力成为一名领航员。她划着贡多拉，带人们穿行于新威尼斯的水道。\n\n这是一部节奏舒缓、充满透明感的作品，适合在想要安静下来的时候阅读。",
    coverTheme: "cover-mint",
    local: false,
  },
];
...
function coverMarkup(item, className = "") {
  ...
  if (blob) {
    return `<img class="${className}" src="${makeObjectUrl(blob)}" alt="${escapeHtml(item.title)}封面" />`;
  }
  ...
  elements.musicList.innerHTML = filtered
    .map((item) => {
      ...
      const cover = item.cover
        ? `<img src="${makeObjectUrl(item.cover)}" alt="" />`
            ...
            ? `
              ...
              <div class="screenshot-strip">
                ${screenshots
                  .map(
                    (image, imageIndex) =>
                      `<img src="${makeObjectUrl(image)}" data-screenshot-index="${imageIndex + 1}" alt="${escapeHtml(item.title)}截图 ${imageIndex + 1}" />`,
                  )
                  .join("")}
              </div>
            `
          ...
          return `
            <div class="manager-item">
              <div class="manager-thumb">
                ${image ? `<img src="${makeObjectUrl(image)}" alt="" />` : escapeHtml(item.title.slice(0, 1))}
              </div>
              <div>
                <strong>${escapeHtml(item.title)}</strong>
...
function setPlayerCover(track) {
  ...
  if (track.cover) {
    elements.playerCover.innerHTML = `<img src="${makeObjectUrl(track.cover)}" alt="" />`;
  } else {
...
async function playTrack(index, autoplay = true) {
  ...
  const source = track.demo ? createDemoWave(normalizedIndex) : URL.createObjectURL(track.audio);
...
async function initialize() {
  ...
  try {
    ...
    state.manga = [
      ...savedManga.sort((a, b) => b.createdAt - a.createdAt),
      ...sampleManga.filter((item) => !state.hiddenIds.has(item.id)),
    ];
    state.music = [
      ...savedMusic.sort((a, b) => b.createdAt - a.createdAt),
      ...sampleMusic.filter((item) => !state.hiddenIds.has(item.id)),
    ];
  } catch (error) {
    console.error(error);
